        ...
        else if(args.length == 1)
        {
            ...
            else
            {
                ...
                if(command != null)
                {
                    ...
                    {
                        sender.sendMessage("§e---------§f 礼物盒子帮助: " + args[0].toLowerCase() + " §e" + getEndFixString(args[0].toLowerCase().length()));
                        sender.sendMessage("§e别名属于§f " + command.commandName);
                    }
                    else
                    {
                        sender.sendMessage("§e---------§f 礼物盒子帮助: " + command.commandName + " §e" + getEndFixString(command.commandName.length()));
                    }
        ...
        for(CommandGrop commandGrop : commandGrops)
        {
            ...
            if (damerauLevenshteinDistance(searchString, commandName.substring(0, searchString.length())) < maxDistance)
            {
                if(!havePossibleMatches)
                {
                    ...
                    sender.sendMessage("§e---------§f 礼物盒子帮助: 搜索 §e--------------------");
